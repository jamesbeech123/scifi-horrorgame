...
public class FirstPersonAIO : MonoBehaviour {
    ...
    public float threatLevel1DetectionChance = 0.2f;
    public float threatLevel2DetectionChance = 0.5f;
    public int healthState {get; private set;} = 3;

    public class ThreatLevelEventArgs : EventArgs {
        public int ThreatLevel { get; set; }
        public Vector3 PlayerPosition { get; set; }
    }
    public event EventHandler<ThreatLevelEventArgs> ThreatLevelChanged;

    protected virtual void OnThreatLevelChanged(Vector3 playerPosition)
    {
        ThreatLevelChanged?.Invoke(this, new ThreatLevelEventArgs {PlayerPosition = playerPosition});
    }
    ...
    private int previousThreatLevel = -1;
    ...
    public void ChangeHealthState(int changeValue)
    {
        healthState -= changeValue;
        Debug.Log($"Health state: {healthState}");
    }

    private bool isSpeedBoostActive = false;
    
    public void SpeedUpPlayer(float speedUpRate, float duration)
    {
        if (isSpeedBoostActive)
        {
            return;
        }
        StartCoroutine(SpeedUpPlayerCoroutine(speedUpRate, duration));
    }

    private IEnumerator SpeedUpPlayerCoroutine(float speedUpRate, float duration)
    {
        isSpeedBoostActive = true;
        float boostDuration = duration;
        float speedMultiplier = 1 + speedUpRate;
    ...
    private IEnumerator UpdateThreatLevelRoutine()
    {
        while (true)
        {
            UpdateThreatLevel();
    ...
    void UpdateThreatLevel()
    {
        ...
        else if (fps_Rigidbody.velocity.magnitude > (walkSpeed-0.01f))
        {
        ...
        // Debug.Log($"Current threat level: {currentThreatLevel}");
        if (GetThreatUpdateChance(currentThreatLevel))
        {
            Vector3 playerPosition = transform.position;
            OnThreatLevelChanged(playerPosition);
            previousThreatLevel = currentThreatLevel;
            Debug.Log($"Threat level: {currentThreatLevel}, Position: {playerPosition}");
        }
        
    }

    private bool GetThreatUpdateChance(int threatLevel)
    {
        switch (threatLevel)
        {
            case 0:
                return false;
            case 1:
                return UnityEngine.Random.Range(0f, 1f) < threatLevel1DetectionChance;
            case 2:
                return UnityEngine.Random.Range(0f, 1f) < threatLevel2DetectionChance;
            case 3:
                return true;
            default:
                return false;
